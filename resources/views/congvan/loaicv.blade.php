...
<div class="content-wrapper">
	...
	<section class="content-header">
		...
		<ol class="breadcrumb">
			...
			<li class="active">Loại công văn</li>
		</ol>
	</section>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-center text-uppercase" id="exampleModalLabel">Thêm mới loại công văn</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="/cong-van/Add_LoaiCV" enctype = "multipart/form-data" method="Post">
                    {{ csrf_field() }}
                    <div class="form-group">
                        <label for="recipient-name" class="col-form-label">Tên loại công văn:</label>
    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Danh sách loại công văn</h3>
      </div>
      <div class="box-body">
          ...
          <table class="table table-bordered">
           <thead>
            <tr>
                ...
                <th class="text-center">Tổng số công văn</th>
             ...
             <tr>
                ...
                <td class="text-center">
                    @for($i = 0; $i < count($count_cv); $i++)
                        @if($count_cv[$i]['LoaiCV_ID'] == $item->ID)
                            {{ $count_cv[$i]['Count'] }}
                        @endif
                    @endfor
                </td>
...
<div class="modal fade edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-center text-uppercase" id="exampleModalLabel">Cập nhật loại công văn</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="/cong-van/Edit_LoaiCV" enctype = "multipart/form-data" method="Post" id="frmEdit">
                    {{ csrf_field() }}
        ...
        $(function () {
            ...
            $('.btnDelete').off('click').on('click', function () {
                ...
                notice.on('pnotify:confirm', () =>
                 $.ajax({
                    data: {},
                    url: '/cong-van/Delete_LoaiCV/' + $(this).data('id'),
                    dataType: 'Json',
                    type: 'GET',
                    success: function () {
                        PNotify.success({
                            title: 'THÔNG BÁO!!',
                            text: 'Xóa loại công văn thành công.'
                        });
                        window.location.href = "/cong-van/loai-cong-van.html";
            ...
            $('.btnEdit').click(function(event) {
                ...
                var ID = $(this).data('id');
                $.ajax({
                    url: "/cong-van/GetLoai_CVByID/" + ID,
                    type: 'GET',
                    dataType: 'json',
                    contentType: "application/json; charset=utf-8",
                    success: function (res) {
                        $('#GhiChu').val(res.query.GhiChu);
                        $('#TenLoai').val(res.query.TenLoai);
                        $('#ID').val(res.query.ID);
                    }
                });
            });
